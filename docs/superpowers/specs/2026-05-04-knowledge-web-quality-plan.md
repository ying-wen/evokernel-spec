# Knowledge Base + Web Quality Plan (v3.31 → v3.32)

> Status: Spec, 2026-05-04. Updated after v3.30 landed on `main` and v3.31
> aligned the public docs/web/API surface.
...
The v3.31 baseline is structurally healthy:

- `pnpm exec tsx scripts/validate-data.ts` passes with 424 entities across 24 entity types.
- `pnpm --filter @evokernel/web test` passes with 49 web tests.
...
- `pnpm audit:data` still reports 3 warnings: suspicious BF16 figures for `mlu220`, `wse-3`, and `rk3588-npu`.
...
- v3.31 brought `/api/index.json`, `/api/health.json`, and `/api/openapi.json` closer to the current API inventory.
...
1. **Data audit hardening**
   - Resolve or explicitly waive the 3 hardware warnings with evidence.
...
3. **Web/API parity**
   - Keep `/api/index.json`, `/api/health.json`, and `/api/openapi.json` aligned with actual API routes.
   - Keep `/techniques/` visible from navigation and homepage discovery as the catalog expands.
   - Convert stale agent/demo pages into either current v3.31 pages or clearly archived legacy examples.
